...
exports.findAll = async(req, res) => {
    ...
    try {
        ...
        console.log(`Problem in reading users, ${user}`)
    }
}
...
exports.update = async(req, res) => {
    ...
    try {
        const result = await User.findOneAndUpdate(
            ...
            updateUser,
            {new: true}  // if document does not exist, it will be created 
                         // (in this case it will not because there are some required fields)
        )
        res.status(200).json({data: result})
